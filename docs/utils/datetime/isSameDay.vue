...
import { isSameDay } from 'buzzts';

const dateA = ref<string | null>('2025-07-08 01:01:01');
const dateB = ref<string | null>('2025-07-08 22:01:01');
const sameDayResult = computed(() => {
  if (dateA.value && dateB.value) {
    return isSameDay(dateA.value, dateB.value);
  }
  return null;
});

const resultText = computed(() => {
  return `isSameDay(${dateA.value}, ${dateB.value}) = ${sameDayResult.value}`;
});
